...
def lasso_loss(X, Y, w, lambd):
    ...
    # loss = np.reshape((np.transpose(Y- X@w)@(Y-X@w))/X.shape[0] + lambd*np.linalg.norm(w, ord=1),(1,1))
    loss = torch.sum(((Y- X@w).t()@(Y-X@w)))/X.shape[0] + lambd*torch.sum(torch.abs(w))
    ### YOUR CODE ENDS HERE ###
...
def lasso_loss_derivative(X, Y, w, lambd):
    ...
    ### YOUR CODE BEGINS HERE ###
    derivative = 2*(X).T@(X @ w - Y)/X.shape[0] + lambd*torch.sign(w)
    ### YOUR CODE ENDS HERE ###
