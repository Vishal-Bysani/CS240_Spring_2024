...
def create_weights(data_dictionary, lambda_val):
    ...
    weights = np.linalg.inv((data_dictionary['X_train']).T@data_dictionary['X_train'] + lambda_val*np.eye(data_dictionary['X_train'].shape[1])) @ (data_dictionary['X_train']).T @ data_dictionary['Y_train']
...
def generate_test_error(data_dictionary, weights):
    ...
    L=(data_dictionary['X_test']@ weights - data_dictionary['Y_test']).squeeze(axis=1)
    test_error= float((L.T@L) / data_dictionary['X_test'].shape[0])
    # END TODO
...
def find_optimal_lambda(error_array, lambda_array):
    ...
    min = error_array[0]
    optimal_lambda = lambda_array[0]
    for i in range(len(lambda_array)):
        if error_array[i]<min:
            min = error_array[i]
            optimal_lambda = lambda_array[i]
