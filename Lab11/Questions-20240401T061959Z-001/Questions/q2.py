    ...
    def get_valid_actions(self):
        ...
        for i in range(self.num_boards):
            if self.active_board_stats[i] == 1:
                valid_actions+= [i * 9 + j for j in range(9) if self.boards[i][j] == '0']
        valid_actions.sort(key=lambda x: (x%9==4, x%9 in [0,2,6,8]), reverse=True)
        return valid_actions

    def is_terminal_history(self):
        # Feel free to implement this in anyway if needed
        return self.is_win() or  len(self.get_valid_actions()) == 0
...
def alpha_beta_pruning(history_obj, alpha, beta, max_player_flag):
    ...
    for action in valid_actions:
        new_object = copy.deepcopy(history_obj)
        new_object.history.append(action)
        new_object.boards = new_object.get_boards()
        new_object.current_player = new_object.get_current_player()
        new_object.active_board_stats = new_object.check_active_boards()
        if max_player_flag:
            value = max(value, alpha_beta_pruning(new_object, alpha, beta, False))
            alpha = max(alpha, value)
            if alpha >= beta:
                break
        else:
            value = min(value, alpha_beta_pruning(new_object, alpha, beta, True))
            beta = min(beta, value)
            if beta <= alpha:
                break
    return value
...
def maxmin(history_obj, max_player_flag):
    ...
    if history_obj.is_terminal_history():
        board_positions_val_dict[board_str] = history_obj.get_value_given_terminal_history()
        return history_obj.get_value_given_terminal_history()
    
    if max_player_flag:
        maxmin_value = -math.inf
    else:
        maxmin_value = math.inf

    valid_actions = history_obj.get_valid_actions()
    for move in valid_actions:
        new_obj = copy.deepcopy(history_obj)
        new_obj.history.append(move)
        new_obj.boards = new_obj.get_boards()
        new_obj.current_player = new_obj.get_current_player()
        new_obj.active_board_stats = new_obj.check_active_boards()
        
        next_maxmin = maxmin(new_obj, not max_player_flag)
        if max_player_flag:
            maxmin_value = max(maxmin_value, next_maxmin)
        else:
            maxmin_value = min(maxmin_value, next_maxmin)
        
    board_positions_val_dict[board_str] = maxmin_value
    return maxmin_value
    # TODO implement
