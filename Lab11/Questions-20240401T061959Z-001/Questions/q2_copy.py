    ...
    def get_valid_actions(self):
        ...
        for i in range(self.num_boards):
            if self.active_board_stats[i] == 1:
                return [i * 9 + j for j in range(9) if self.boards[i][j] == '0']
        return None

    def is_terminal_history(self):
        # Feel free to implement this in anyway if needed
        return self.is_win()!=0 or self.get_valid_actions() ==None or len(self.get_valid_actions()) == 0

    def get_value_given_terminal_history(self):
        # Feel free to implement this in anyway if needed
        if self.get_valid_actions() ==None or len(self.get_valid_actions()) == 0:
            return 0
        if self.is_win()==1:
           return 1
        if self.is_win()==2:
