...
def inv_transform(distribution: str, num_samples: int, **kwargs) -> list:
    ...
    if distribution=="exponential":
        samples=list(np.round(np.random.exponential(scale=1/float(kwargs["lambda"]),size=num_samples),4))    
    elif distribution=="cauchy":
        samples=np.random.uniform(size=num_samples)
        samples = list(np.round(kwargs["peak_x"]+kwargs["gamma"]*np.tan(np.pi*(samples-0.5)) ,4))
    # END TODO
...
if __name__ == "__main__":
    ...
    for distribution in ["cauchy", "exponential"]:
        ...
        plt.figure()
        plt.hist(samples, bins=100, color='c', edgecolor='black')
