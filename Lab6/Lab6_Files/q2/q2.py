...
class SKLearnDecisionTree:
    ...
    def train_model(self, criterion="entropy"):
        ...
        results["f1_train_score"]=metrics.f1_score(self.Y_train, self.tree.predict(self.X_train))
        results["f1_test_score"]=metrics.f1_score(self.Y_test, self.tree.predict(self.X_test))
        return results
        ### Student Code end
        ...
        for max_depth in range(1,11):
            for min_samples_split in range(3,31,3):
                for min_samples_leaf in range(1,11):
                    self.tree = DecisionTreeClassifier(criterion=criterion, random_state=42, min_samples_leaf=min_samples_leaf, min_samples_split=min_samples_split,max_depth=max_depth)
                    self.tree.fit(self.X_train,self.Y_train)
