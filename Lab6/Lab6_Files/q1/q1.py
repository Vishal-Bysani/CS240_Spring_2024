...
def gini_index(classes):
    ...
    unique_labels, probs = np.unique(classes, return_counts=True)
    probs = probs/ len(classes)
    gini_impurity_value = (float)(1 - probs@probs)
...
def entropy(classes):
    ...
    _, probs = np.unique(classes, return_counts=True)
    probs = probs/ len(classes)
    if len(_) == 1:
        return 0
    entropy_value = (float)(-np.sum(probs* np.log(probs+1e-12)/np.log(len(_))))
...
def information_gain(examples, classes, attr):
    ...
    unique_labels, probs= np.unique(examples[:,attr],return_counts=True)
    probs = probs/ len(classes)
    H_Y_given_X = 0

    for i in range(len(unique_labels)):
        H_Y_given_X += probs[i]*entropy(classes[examples[:,attr]==unique_labels[i]])
    
    H_Y = entropy(classes)
    # Calculate and return the information gain
    information_gain_value = H_Y - H_Y_given_X
...
def gini_gain(example, classes, attr):
    ...
    unique_labels, probs = np.unique(example[:,attr],return_counts=True)
    probs = probs/ len(classes)
    gini_Y_given_X = 0
    for i in range(len(unique_labels)):
        gini_Y_given_X = gini_Y_given_X + probs[i]*gini_index(classes[example[:,attr]==unique_labels[i]])
    gini_Y = gini_index(classes)

    # Calculate and return the gini gain
    gini_gain_value = gini_Y - gini_Y_given_X
...
class DecisionTree:
    ...
    def __build_tree__(self, features, classes, attributes, depth=0):
        ...
        if (len(attributes) == 0):
            ...
            for i in attributes:
                if information_gain(features, classes, i) > information_gain(features, classes, best_attr):
                    best_attr = i
            ...
            # Split the dataset based on the unique values of the best attribute.
                children[i] = self.__build_tree__(features[features[:, best_attr] == i], classes[features[:, best_attr] == i],children_attr.copy(), depth-1)
            
            # Determine the default prediction
            if 2*np.sum(classes)>= len(classes):
                default_for_empty = 1
            else:
                default_for_empty = 0
