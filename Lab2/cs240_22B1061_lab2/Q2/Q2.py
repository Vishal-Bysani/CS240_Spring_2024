...
def w_closed_form(X, Y):
    ...
    w_closed =torch.round(torch.matmul(torch.linalg.inv(torch.matmul(X.t(),X)),torch.matmul(X.t(),Y)),decimals=4)
    # Student code end
...
def transform_features(X, degree=1):
    ...
    phi_X= torch.linalg.vander(X.view(-1),N=degree+1)
    # Student code end
...
def l2_loss(X, Y, w):
    ...
    P= (Y- torch.matmul(X,w)).view(-1)
    loss= np.float64(torch.dot(P,P)/X.shape[0])
    # Student code end
...
def l2_loss_derivative(X, Y, w):
    ...
    # Please take care of normalization factor 1/n
    derivative = 2*torch.matmul(X.t(), torch.matmul(X,w)-Y)/X.shape[0]
    # Student code end
...
def train_model(X_train, Y_train, X_test, Y_test, w, eta):
    ...
    while abs(l2_loss(X_train,Y_train,w) - old_loss) > epsilon :
        ...
        w=w- eta*l2_loss_derivative(X_train,Y_train,w)
