...
class NeuralNetwork:
    ...
    def cost(self, training_data):
        ...
        for x, y in training_data:
            cost += 0.5 * np.sum((self.forward_pass(x) - y) ** 2)
        # TODO calculate costs
    ...
    def forward_pass(self, x):
        ...
        for i in range(len(self.weights)):
            activation = self.sigmoid(self.weights[i].T@activation + self.biases[i])
            nn_output = activation
        # TODO do a forward pass of the NN and return the final output
        return nn_output
        ...
        for j in range(epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for data_point in mini_batch:
                    t = self.back_propagation(data_point[0], data_point[1])
    ...
    def back_propagation(self, x, y):
        ...
        activation = x
        for i in range(len(self.weights)):
            z_value = (self.weights[i].T@activation + self.biases[i])
            activation = self.sigmoid(z_value)
            a.append(activation)
            z.append(z_value)
        ...
        da_dz = self.sigmoid_derivative(z[-1])
        dz_dw = a[-2]
        dC_dw[-1] = dz_dw @ (dC_da * da_dz).T
        dC_db[-1] = dC_da * da_dz

        for i in range(2, len(self.num_neurons_per_layer_list)):
            dC_da = self.weights[-i+1] @ (dC_da * da_dz)
            da_dz = self.sigmoid_derivative(z[-i])
            dz_dw = a[-i-1]
            dC_dw[-i] = dz_dw @ (dC_da * da_dz).T
            dC_db[-i] = dC_da * da_dz
