...
class ConvolutionLayer:
    ...
    def forward(self, input):
        ...
        ## TODO
        output = np.zeros(shape=(self.num_filters, height - self.filter_size+1, width -self.filter_size+1))
    ...
    def backward(self, output_error, learning_rate):
        ...
        filter_error = np.zeros(self.conv_filter.shape)
        for f in range(self.num_filters):
            for i in range(self.filter_size):
                for j in range(self.filter_size):
                    filter_error[f][i][j] = np.sum(output_error[f] * self.input[i:i+output_error.shape[1], j:j+output_error.shape[2]])

        bias_error = np.sum(output_error, axis=(1, 2))
...
class ReLULayer:
    ...
    def backward(self, output_error, learning_rate):
        ...
        ## TODO
        input_error = output_error * np.where(self.input>0 , 1, 0)
        ## END TODO
...
class MaxPoolingLayer:
    ...
    def forward(self, input):
        ...
        ## TODO
        output = np.zeros(shape =(num_filters, height//self.filter_size, width//self.filter_size))
        for f in range(num_filters):
            for i in range(output.shape[1]):
                for j in range(output.shape[2]):
                    output[f][i][j] = np.max(input[f, i*self.filter_size:(i+1)*self.filter_size, j*self.filter_size: (j+1)*self.filter_size]) 
        ## END TODO
    ...
    def backward(self, output_error, learning_rate):
        ...
        for f in range(self.input.shape[0]):
            for i in range(output_error.shape[1]):
                for j in range(output_error.shape[2]):
                    a = self.input[f, i*self.filter_size:(i+1)*self.filter_size, j*self.filter_size: (j+1)*self.filter_size]
                    b = np.zeros_like(a)
                    b[np.unravel_index(np.argmax(a, axis=None), a.shape)] = 1
                    input_error[f, i*self.filter_size:(i+1)*self.filter_size, j*self.filter_size: (j+1)*self.filter_size] = b*output_error[f][i][j]
...
class FlattenLayer:
    ...
    def forward(self, input):
        ...
        ## TODO
        output = np.copy(input).reshape((input.shape[0]*input.shape[1]*input.shape[2], 1))
        ## END TODO
    ...
    def backward(self, output_error, learning_rate):
        ...
        ## TODO
        input_error = np.copy(output_error).reshape(self.input_shape)
