...
def log_loss(y, y_dash):
    ...
    ### YOUR CODE BEGINS HERE ###
    loss = (float)(-y*np.log(y_dash)-(1-y)*np.log(1-y_dash))
    ### YOUR CODE ENDS HERE ###
...
def cost_logreg(X, y, w, w_0):
    ...
    ### YOUR CODE BEGINS HERE ###
    y_hat = logistic(X@w+w_0)
    cost=(float)(-(y@np.log(y_hat)+(1-y)@np.log((1-y_hat)))/m)
    ### YOUR CODE ENDS HERE ###
...
def grad_logreg(X, y, w, w_0):
    ...
    ### YOUR CODE BEGINS HERE ###
    grad_w_0=(float)(np.sum(logistic(X@w+w_0)-y)/m)
    grad_w = X.T@(logistic(X@w+w_0)-y)/m
