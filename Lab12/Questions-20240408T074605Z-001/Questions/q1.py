...
def psne_gen(num_players, strategy, util_matrix):
    ...
    for i in range(len(allpermut)):
        ...
        for j in range(num_players):
            temp1=temp[:]
            temp1.pop(j)
            for k in range(strategy[j]):
                temp1.insert(j,k)
                if util_matrix[tuple(temp)][j]<util_matrix[tuple(temp1)][j]:
                    flag=1
                    break
                temp1.pop(j)
            if flag==1:
...
def msne_gen(num_players, strategy, util_matrix):
    ...
    p1 = (util_matrix[1][1][0] - util_matrix[1][0][0])/(util_matrix[0][0][0] - util_matrix[1][0][0] + util_matrix[1][1][0] - util_matrix[0][1][0]) 

    q1 = (util_matrix[1][1][1] - util_matrix[1][0][1])/(util_matrix[0][0][1] - util_matrix[1][0][1] + util_matrix[1][1][1] - util_matrix[0][1][1])
